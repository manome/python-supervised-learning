...
def main():
    # Load the dataset
    FILE_PATH = 'local_data_sample.csv'
    EXCLUDE_COLUMNS = [0]

    df = pd.read_csv(FILE_PATH)
    x = df.drop(df.columns[EXCLUDE_COLUMNS], axis=1).iloc[:, :-1].values
    y = df.iloc[:, -1].values
    feature_names = list(df.drop(df.columns[EXCLUDE_COLUMNS], axis=1).columns[:-1])
